...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      const ret = {
        id: commit,
        url: 'https://github.com/brianzliu/dsc106-portfolio/commit/' + commit,
        author,
        date,
        time,
        timezone,
...
function createBrushSelector(svg) {
  ...
  // Ensure dots and overlay-following elements are above the brush overlay
...
function renderScatterPlot(data, commits) {
  ...
  // Gridlines (added before axes so they sit underneath)
  const gridlines = svg
    .append('g')
  ...
  // Radius scale based on total lines edited
  const [minLines, maxLines] = d3.extent(commits, (d) => d.totalLines);
  const rScale = d3.scaleSqrt().domain([minLines, maxLines]).range([3, 30]);

  // Sort by total lines so larger dots render first
...
function updateScatterPlot(data, commits) {
  ...
  // Clear out the existing x-axis and re-render it in place.
